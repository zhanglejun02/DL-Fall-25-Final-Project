...
from galore_torch import GaLoreAdamW, HMTAdamW, ApolloAdamW, Lotus, FiraAdamW
...
def parse_args():
    ...
    parser.add_argument("--optimizer_name",
                        ...
                        choices=["Galore", "Apollo", "Lotus","Fira"])
    parser.add_argument("--threshold", type=float, default=0.01)
    parser.add_argument("--check_interval", type=int, default=50)
    parser.add_argument("--proj_method", type=str, default="traditional", choices=["traditional", "oversample"])
...
def main():
    ...
    if not args.enable_galore:
        ...
        if args.optimizer_name == "Galore":
            ...
            logger.info(f"The Optimizer is GaLoreAdamW")
        elif args.optimizer_name == "HMT":
            optimizer = HMTAdamW(param_groups, lr=args.learning_rate)
            logger.info(f"The Optimizer is HMTAdamW")
        elif args.optimizer_name == "Apollo":
            optimizer = ApolloAdamW(param_groups, lr=args.learning_rate)
            logger.info(f"The Optimizer is Apollo")
        elif args.optimizer_name == "Lotus":
            optimizer = Lotus(param_groups, lr=args.learning_rate)
            logger.info(f"The Optimizer is Lotus")
        elif args.optimizer_name == "Fira":
            optimizer = FiraAdamW(param_groups, lr=args.learning_rate)
    ...
    logger.info(f"Execution time: {cost:.2f} seconds")
    ...
    if args.enable_galore and accelerator.is_main_process:
        projector = None
        ...
        if projector is not None and args.optimizer_name == "Lotus":
            logger.info(f"[End of Training] => total subspace updates = {projector.update_count}")
            logger.info('\n'.join(map(str, projector.information)))
        else:
            logger.info("No projector found in unwrapped_optimizer state.")
        
        if args.use_stable:
            logger.info("Trick is used")
        else:
            logger.info("No trick")

        if args.proj_method == "traditional":
            logger.info("Traditional method")
        else:
            logger.info("Oversample method")
...
if __name__ == "__main__":
    ...
    log_file_path = f'logfiles/{method}/{args.model_name_or_path}/{args.task_name}_{args.lora_r}_{args.threshold}_{args.check_interval}.log'
