...
from galore_torch import GaLoreAdamW, GaLoreAdamW8bit, GaLoreAdafactor, Lotus, Lotus8bit
...
def parse_args(args):
    ...
    parser.add_argument("--proj_method", type=str, default="iteration", choices=["traditional", "iteration"])
    parser.add_argument("--use_stable", 
                        action="store_true",
                        help="Whether or not to use stable in the subspace update.")
...
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size):
    ...
    # "json",
    # data_files="/root/autodl-tmp/Lotus/c4/en/c4-validation.00000-of-00008.json.gz",
    # split="train[:10%]"
    # )
    val_data = datasets.load_dataset("c4/en", split="validation") #DGX
    # val_data = load_from_disk("datasets/en/default/0.0.0/37935baca3928790")["validation"]
...
def main(args):
    ...
    # "json",
    # data_files="/root/autodl-tmp/Lotus/c4/en/c4-train.00000-of-01024.json.gz",
    # split="train"
    # )
    # data = dataset.select(range(100))

    data = datasets.load_dataset("c4/en", split="train")
    # data =  load_from_disk("datasets/en/default/0.0.0/37935baca3928790")
    seed_for_shuffle = 42 
    ...
    if 'galore' in args.optimizer.lower() or "Lotus" in args.optimizer:
        ...
        param_groups = [{'params': regular_params}, 
                        {'params': galore_params, 'rank': args.rank, 'update_proj_gap': args.update_proj_gap, 'scale': args.galore_scale, 'proj_type': args.proj_type, 
                         "use_stable": args.use_stable, "drift_threshold":args.threshold, "check_interval": args.check_interval, "proj_method": args.proj_method}]
    ...
    elif args.optimizer.lower() == "Lotus8bit":
        optimizer = Lotus8bit(param_groups, lr=args.lr, weight_decay=args.weight_decay)
    ...
    elif args.optimizer.lower() == 'Lotus8bit_per_layer':
        # TODO: seems scheduler call twice in one update step, need to check, for now double the num_training_steps, warmup_steps and update_proj_gap
        optimizer_dict = {}
        for p in model.parameters():
            if p.requires_grad:
                if id(p) in id_galore_params:
                    optimizer_dict[p] = Lotus8bit([{'params': [p], 'rank': args.rank, 'update_proj_gap': args.update_proj_gap * 2, 'scale': args.galore_scale, 'proj_type': args.proj_type}], lr=args.lr, weight_decay=args.weight_decay)
                else:
                    optimizer_dict[p] = bnb.optim.Adam8bit([p], lr=args.lr, weight_decay=args.weight_decay)
        ...
        if global_rank == 0:
            for group in unwrapped_optimizer.param_groups:
                for p in group["params"]:
                    if p in unwrapped_optimizer.state and "projector" in unwrapped_optimizer.state[p]:
                        projector = unwrapped_optimizer.state[p]["projector"]
                        break
            ...
            wandb.log({
                ...
                "projector information": projector.project_times,
                "update_count": projector.update_count,
                "diff_val": float(projector.drift_val)
                },
                step=global_step,
            )
        update_time = time.time()
